...
class Solution(object):
    def licenseKeyFormatting(self, S, K):
        ...
        :rtype: str
        """
        S = S.upper().replace('-', '')[::-1] # uppercase, replace - with nothing, slice[start:stop:stpes] -1 backward
        # reverseing to make the first block not included in the patter we are applying to, sort of using it as mod
        # join() takes all items in iterable and joins them into one string
